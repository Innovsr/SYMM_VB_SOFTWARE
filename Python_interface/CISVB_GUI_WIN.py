...
#        print('geometry:',self.symat_py, self.coordx_py, self.coordy_py, self.coordz_py, self.symatno_py)
    ...
    def share_input_data(self, output_folder):
        symm_str = ctypes.CDLL("/home/sourav/Desktop/SYMM_VB_SOFTWARE/Python_interface/symm_str.dll")

        # Function argument data-types:
        symm_str.get_ctrl_inputs.argtypes = [
            ctypes.c_char * 5,                      # geometry_unit
            ctypes.c_int,                            # nao_py
            ctypes.c_int,                            # nae_py
            ctypes.c_int,                            # nmul
            ctypes.c_char * 100,                     # output_file_name
            ctypes.c_int,                            # chinst
            ctypes.c_int,                            # symm_py
            ctypes.c_int,                            # set_order_py
            ctypes.c_int,                            # nset_py
            ctypes.c_int,                            # mout_py
            ctypes.c_int,                            # ovlp_py
            ctypes.c_int,                            # itb_py
            ctypes.c_int,                            # nnb_py
            ctypes.c_int,                            # syb_py
            ctypes.c_int,                            # mnbond_py
            ctypes.c_int,                            # radical_py
            ctypes.c_int,                            # nmbond_py
            ctypes.c_char * 5 * 100,                 # symat_array
            np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags="C_CONTIGUOUS"),  # coordx
            np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags="C_CONTIGUOUS"),  # coordy
            np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags="C_CONTIGUOUS"),  # coordz
            np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags="C_CONTIGUOUS"),  # symatno_array
            np.ctypeslib.ndpointer(dtype=np.int32, ndim=2, flags="C_CONTIGUOUS"),    # atoset_array
            np.ctypeslib.ndpointer(dtype=np.int32, ndim=1, flags="C_CONTIGUOUS"),    # norbsym_array
            np.ctypeslib.ndpointer(dtype=np.int32, ndim=1, flags="C_CONTIGUOUS"),    # atn_array
            np.ctypeslib.ndpointer(dtype=np.int32, ndim=2, flags="C_CONTIGUOUS"),    # orbsym_array
            np.ctypeslib.ndpointer(dtype=np.int32, ndim=1, flags="C_CONTIGUOUS"),    # active_atom_array
            ctypes.c_int,                            # flgst_py
            ctypes.c_int,                            # total_atoms
            ctypes.c_int,                            # niao_py
            ctypes.c_int,                            # active_atm_num
            ctypes.c_char * 300                      # output_folder
        ]


        
        # Set return type (None if it's a subroutine)
        symm_str.get_ctrl_inputs.restype = None  
